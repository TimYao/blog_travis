...
## https验证流程
  ...
  http作为应用层，内容明文发送，导致过程中不安全易被劫持且篡改，https为http+tsl结合，tsl提供了ssl加密。tsl为ssl的标准化版本。
...
- 同域下通信
  ...
  通过建立一个共享工作线程来进行与多个窗口，多个Iframe间的通信，主要思想还是建立了共享访问线程来进行，通过postMessage,onmessage来完成发送与接收
